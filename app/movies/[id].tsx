...
const MovieInfo=({label,value}:MovieInfoProps)=>(
  <View className='flex-col items-start justify-center mt-5 px-2'>
    ...
    <Text className='text-light-100 font-bold text-sm mt-2'>{value || 'No Details'}</Text>
  </View>
)
  ...
  return (
    <View className='bg-primary flex-1'>
      <ScrollView contentContainerStyle={{
        ...
        <View className='flex-row items-center bg-dark-100 px-2 py-1 rounded-md self-start gap-x-1 mt-2'>
        ...
        <View className='flex flex-row justify-between w-1/2'>
          <MovieInfo label="Budget" value={`$${movie?.budget/1_000_000 || 'N/A'} millions`}/>
          <MovieInfo label='Revenue' value={`$${(Math.round(movie?.revenue)/1_000_000).toFixed(2)} millions`}/>
        </View>
        <MovieInfo label='Production Companies' value={movie?.production_companies.map((c)=>c.name).join('-') || 'N/A'}/>
      </ScrollView>

      <TouchableOpacity className='absolute bottom-5 left-0 right-0 mx-5 bg-accent rounded-lg py-3.5 flex
                                   flex-row items-center justify-center z-50'
                                   onPress={router.back}>
